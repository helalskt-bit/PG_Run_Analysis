# PG_Run_Analysis_ST_v7_6_smooth.py
...
if folder_upload and reference_file:
    if st.sidebar.button("Analysis PG run"):
        ...
        # Step 2: Filter uploaded files by Reference (smoother progress)
        ...
        total_chunks = 0
        for uploaded_file in folder_upload:
            name, ext = os.path.splitext(uploaded_file.name)
            if ext.lower() in ['.xlsx', '.xls']:
                xls = pd.ExcelFile(uploaded_file)
                total_chunks += sum(1 for sheet in xls.sheet_names if sheet in ['Down Data', 'Power Alarms'])
            elif ext.lower() == '.csv':
                total_chunks += sum(1 for _ in pd.read_csv(uploaded_file, chunksize=chunk_size))
            ...
            if ext.lower() in ['.xlsx', '.xls']:
                ...
                for sheet in xls.sheet_names:
                    if sheet in ['Down Data','Power Alarms']:
                        ...
                        processed_chunks += 1
                        st.session_state.progress_val = 0.125 + 0.5 * (processed_chunks/total_chunks)
                        progress.progress(st.session_state.progress_val)
            elif ext.lower() == '.csv':
